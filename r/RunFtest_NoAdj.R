...
expGQ<-function(gene){
  ...
  lmCOPD<-lm(expr ~ 1, data = phenoSub[phenoSub$copd=="yes",])
  lmControl<-lm(expr ~ 1, data = phenoSub[phenoSub$copd=="no",])
...
write.table(Ffram, file.path(baseDir, "COPD_DiffVarResults_NoAdj.txt"), row.names = FALSE, col.names = TRUE)

...
pheno<-samples[, c("sampGEO", "copd", "age", "sex", "smoking", "asthma", "inhaledMedications", "fev1", "fev1.fvc")]
...
###gene expression = fev1 + age + sex + smoking + asthma + inhaledMedications
...
expLM<-function(gene){
  ...
  lmOut<-summary(lm(expr ~ fev1.fvc + age + sex + smoking + asthma + inhaledMedications, data = phenoSub))
  
  coefOut<-lmOut$coefficients["fev1.fvc",]
  return(coefOut)}
...
colnames(DGEfram)<-c("gene", "beta","se", "testStat", "pvalue")

write.table(DGEfram, file.path(baseDir, "fev1fvc_DGEresults.txt"), row.names = FALSE, col.names = TRUE)

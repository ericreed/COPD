...
expLM<-function(gene){
  ...
  lmOut<-summary(lm(expr ~ fev1 + age + sex + smoking + asthma + inhaledMedications, data = phenoSub))
  
  coefOut<-lmOut$coefficients["fev1",]
  return(coefOut)}
...
write.table(DGEfram, file.path(baseDir, "fev1_DGEresults.txt"), row.names = FALSE, col.names = TRUE)
